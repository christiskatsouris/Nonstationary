...
Generally, the framework of the cointegration model allows to capture various dynamics and thus investigate international finance and international macroeconomic puzzles such as the Purchasing Power Parity (PPP) condition. In particular, the PPP theory states that in the long run, the exchange rate between two countries is determined by their relative price levels (see, Chapter 6 in Engel, Mark and West (2007) and Deardorff (1979) for related definitions). The PPP theory puzzle provides a suitable economic theory along with time series observations that exhibit cointegrated dynamics and therefore allows for the implementation of different econometric techniques such as testing for unit roots, testing for the presence of unit roots at the conditional quantile distributions, testing for the presence of structural breaks and testing for cointegration (see, Nelson and Plosser (1982) and Corbae and Ouliaris (1988)). 
