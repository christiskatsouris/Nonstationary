...
In order to obtain insights regarding the finite-sample as well as large-sample performance of a testing methodology, we can conduct a Monte Carlo simulation study. Specifically, with simulation experiments we obtain the empirical size (under the null hypothesis of no parameter instability) and power function (under the alternative hypothesis of a structural break at an unknown location) for the test statistic(s) under investigation. Since we focus on the sequential monitoring procedures then an additional metric we can also rely on, for evaluating how well these test statistics perform under different experimental conditions is the average run length (ARL), which is obtained under the alternative of a structural break within the monitoring period and by comparing these distributions with respect to different break-point locations (e.g., early change, structural change in the middle, late change).  
...
$\textbf{(B).}$ $\textbf{Testing for Multiple Structural Breaks}$
...
$\textbf{(E).}$ $\textbf{Testing for autocorrelation in time series models}$
